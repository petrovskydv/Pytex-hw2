...
Основное приложение создаёт `AIOKafkaProducer` в lifespan и публикует каждую сгенерированную покупку как факт
`tickets.purchased`. Producer использует `KAFKA__LINGER_MS=75`, чтобы короткое время накапливать события перед
сетевой отправкой. При shutdown сначала останавливается генератор, затем Kafka producer сбрасывает накопленные
